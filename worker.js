...
// (Classic Edition - Feishu Webhook, TG Bot Commands, Cost Tracking)
...
<body class="bg-slate-50 text-slate-800 dark:bg-darkbg dark:text-slate-200 min-h-screen flex flex-col">
        ...
        <main class="flex-1 md:ml-64 w-full flex flex-col">
            ...
            <div class="max-w-7xl mx-auto p-4 md:p-8 w-full flex-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div class="bg-gradient-to-r from-purple-500 to-pink-500 text-white p-5 rounded-xl shadow-lg flex justify-between items-center"><div><div class="text-purple-100 text-xs font-bold uppercase">{{.T.Cost30}}</div><div class="text-3xl font-bold">{{.Stats.ProjectedCost}}</div></div><div class="text-purple-300 text-4xl opacity-50"><i class="fas fa-wallet"></i></div></div>
                </div>
                        ...
                        <table class="w-full text-left border-collapse min-w-[700px]" id="assetTable">
                            ...
                            <tbody class="text-sm">
                                ...
                                <tr class="border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/50 search-item transition-all duration-200">
                                    ...
                                    <td class="py-3 font-mono text-slate-500">{{.Cost}}</td>
                                    ...
                                    <td class="py-3 text-right pr-2 space-x-2"><button onclick="openView('{{.Name}}', '{{.Detail}}', '{{.Link}}')" class="text-green-600 bg-green-50 dark:bg-green-900/30 px-2.5 py-1.5 rounded-lg text-xs font-medium"><i class="fas fa-eye"></i></button><button onclick="openEdit('{{.ID}}','{{.Category}}','{{.Name}}','{{.Date}}','{{.Cost}}','{{.Link}}','{{.Detail}}')" class="text-blue-500 p-1"><i class="fas fa-edit"></i></button><form action="/item/del" method="POST" onsubmit="return confirm('{{$.T.ConfirmDel}}')" class="inline"><input type="hidden" name="id" value="{{.ID}}"><button class="text-slate-400 hover:text-red-500 p-1"><i class="fas fa-trash-alt"></i></button></form></td>
                                </tr>
                                {{end}}
                            </tbody>
                        </table>
    ...
    <div id="addModal" class="fixed inset-0 hidden items-center justify-center z-50 bg-black/50 backdrop-blur-sm"><div class="glass-card p-6 rounded-2xl w-full max-w-lg shadow-2xl"><h3 class="font-bold mb-4 text-lg">{{.T.Add}}</h3><form action="/item/add" method="POST" class="space-y-4"><input type="text" name="category" list="cat_list" placeholder="{{.T.Ph_Cat}}" class="w-full p-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600 outline-none" required><datalist id="cat_list">{{range .UserCats}}<option value="{{.}}">{{end}}</datalist><input type="text" name="name" placeholder="{{.T.Name}}" class="w-full p-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600 outline-none" required><div class="flex gap-2"><input type="date" name="date" class="w-1/2 p-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600 outline-none" required><input type="number" step="0.01" name="cost" placeholder="{{.T.Cost}} (Opt)" class="w-1/2 p-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600 outline-none"></div><input type="url" name="link" placeholder="{{.T.Link}} (Opt)" class="w-full p-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600 outline-none"><textarea name="detail" rows="3" placeholder="{{.T.Details}}" class="w-full p-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600 font-mono text-sm outline-none"></textarea><div class="flex gap-3"><button type="button" onclick="closeModal('addModal')" class="flex-1 py-3 bg-slate-100 text-slate-600 rounded-xl font-bold">{{.T.Cancel}}</button><button type="submit" class="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold">{{.T.Save}}</button></div></form></div></div>
    
    <div id="editModal" class="fixed inset-0 hidden items-center justify-center z-50 bg-black/50 backdrop-blur-sm"><div class="glass-card p-6 rounded-2xl w-full max-w-lg shadow-2xl"><h3 class="font-bold mb-4 text-lg">{{.T.Edit}}</h3><form action="/item/update" method="POST" class="space-y-4"><input type="hidden" name="id" id="edit_id"><input type="text" name="category" id="edit_category" list="cat_list" class="w-full p-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600 outline-none" required><input type="text" name="name" id="edit_name" class="w-full p-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600 outline-none"><div class="flex gap-2"><input type="date" name="date" id="edit_date" class="w-1/2 p-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600 outline-none"><input type="number" step="0.01" name="cost" id="edit_cost" class="w-1/2 p-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600 outline-none"></div><input type="url" name="link" id="edit_link" class="w-full p-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600 outline-none"><textarea name="detail" id="edit_detail" rows="3" class="w-full p-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600 font-mono text-sm outline-none"></textarea><div class="flex gap-3"><button type="button" onclick="closeModal('editModal')" class="flex-1 py-3 bg-slate-100 text-slate-600 rounded-xl font-bold">{{.T.Cancel}}</button><button type="submit" class="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold">{{.T.Save}}</button></div></form></div></div>
    ...
    <script>
        ...
        function openEdit(id,c,n,d,cost,l,det){
            ...
            document.getElementById('edit_cost').value=cost; document.getElementById('edit_link').value=l;
            document.getElementById('edit_detail').value=det; openModal('editModal');
        }
        ...
        function sortNumber(col) { 
            ...
            const rows = Array.from(tb.rows); 
            sortDir = !sortDir; 
            rows.sort((a,b) => (parseFloat(a.cells[col].innerText||0) - parseFloat(b.cells[col].innerText||0)) * (sortDir ? 1 : -1)); 
            tb.append(...rows); 
        }
    </script>
</body>
<div id="importModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white dark:bg-darkcard rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl">
        ...
        <form action="/import" method="POST" enctype="multipart/form-data">
            <input type="file" name="file" accept=".json" required class="w-full p-2.5 border rounded-lg dark:bg-slate-800 dark:border-slate-600 mb-4">
            <div class="flex gap-2">
                <button type="submit" class="flex-1 bg-blue-600 text-white py-2 rounded-lg font-bold hover:bg-blue-700">导入</button>
                <button type="button" onclick="closeModal('importModal')" class="flex-1 bg-slate-200 dark:bg-slate-700 py-2 rounded-lg font-bold">取消</button>
            </div>
        </form>
    </div>
</div>
...
async function handleSetLang(request, storage) {
    ...
    return new Response('', { 
        status: 302, 
        headers: { 
            'Location': new URL(targetPath, request.url).toString(), 
            'Set-Cookie': setCookie('lang', lang, 31536000) 
        } 
    });
}
...
async function handleHome(request, storage, user) {
    ...
    items.forEach(i => { 
        ...
        if (diffDays <= 30 && diffDays >= 0) { 
            stats.Expiring++; 
            stats.ProjectedCost += parseFloat(i.Cost || 0); 
        }
...
async function handleTestNotify(request, storage, user) {
    ...
    // 检查 Telegram 配置
    if (!s.tg_token) {
        errorMsg = '未配置 Telegram Bot Token';
    } else if (!user.ChatID) {
        errorMsg = '未填写 Telegram Chat ID';
    } else {
        try {
            ...
            errorMsg = 'Telegram 发送失败: ' + e.message;
        }
    }
